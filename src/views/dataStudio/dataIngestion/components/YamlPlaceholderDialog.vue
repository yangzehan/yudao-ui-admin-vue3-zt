...
const defaultHints: Record<string, string> = {
  ...
  flink_host: 'Flink 集群地址',
  flink_port: 'Flink 端口',
  flink_job_name: 'Flink 作业名称',
  table_name: '表名称',
  schema_name: 'Schema 名称'
}
...
const handleDatasourceChange = async (placeholder: string, datasourceId: number) => {
  ...
  try {
    ...
    // 如果有表名占位符，自动填充第一个表名
    const tablePlaceholder = placeholderList.value.find(p => p.endsWith('_table') || p === 'target_table' || p === 'source_table')
